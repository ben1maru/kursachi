...
public class AdminViews {
    ...
    @FXML
    void initialize() {
        categoriesBtn.setOnAction(event -> loadRecords("category"));
        clientsBtn.setOnAction(event -> loadRecords("Clients"));
    ...
    private void loadRecords(String tableName) {
        ...
        try (Connection conn = db.getDbConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT id, " + (tableName.equals("Clients") ? "PIB AS name" : "name") + " FROM " + tableName);
    ...
    private HBox createRecordBox(Record record) {
        ...
        recordBox.setStyle("-fx-background-color: #FFFFFF; -fx-border-color: #328E6E; -fx-border-width: 2; -fx-border-radius: 5; -fx-background-radius: 5; -fx-padding: 10;");
        recordBox.setMaxWidth(Double.MAX_VALUE);

        Label nameLabel = new Label(record.getName());
        nameLabel.setStyle("-fx-font-size: 14; -fx-text-fill: #328E6E;");
        ...
        editBtn.setStyle("-fx-background-color: #90C67C; -fx-text-fill: white; -fx-font-size: 12; -fx-border-radius: 5; -fx-background-radius: 5;");
        editBtn.setOnAction(event -> handleEdit(record));

        Button deleteBtn = new Button("Видалити");
        deleteBtn.setStyle("-fx-background-color: #E74C3C; -fx-text-fill: white; -fx-font-size: 12; -fx-border-radius: 5; -fx-background-radius: 5;");
    ...
    private void handleEdit(Record record) {
        try {
            String fxmlFile = record.getTableName().equals("Clients")
                    ? "EditClientViews.fxml"
                    : "EditCategoryViews.fxml";

            FXMLLoader loader = new FXMLLoader(getClass().getResource(fxmlFile));
            Parent root = loader.load();

            if (record.getTableName().equals("Clients")) {
                EditClientViews controller = loader.getController();
                controller.setClientId(record.getId());
            } else {
                EditCategoryViews controller = loader.getController();
                controller.setCategoryId(record.getId());
            }
            ...
            showAlert("Помилка", "Не вдалося завантажити форму редагування", Alert.AlertType.ERROR);
        }
    }
    ...
    private void handleDelete(Record record) {
        ...
        try (Connection conn = db.getDbConnection()) {
            ...
            if (record.getTableName().equals("category")) {
                String deleteProductsQuery = "DELETE FROM Product WHERE id = ?";
                try (PreparedStatement stmt = conn.prepareStatement(deleteProductsQuery)) {
                    stmt.setInt(1, record.getId());
                    stmt.executeUpdate();
                }
            } else if (record.getTableName().equals("Clients")) {
